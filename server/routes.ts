...
const sponsorLogoStorage = multer.diskStorage({
  destination: (req, file, cb) => {
    const uploadDir = path.resolve(process.cwd(), "attached_assets/sponsor_logos");
  ...
  filename: (req, file, cb) => {
    ...
    cb(null, `sponsor-logo-${uniqueSuffix}${ext}`);
  }
});
...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.get('/api/venues', async (req, res) => {
    try {
      ...
      query += `
        GROUP BY v.id, v.name, v.name_ar, v.city, v.location, v.image_url
  ...
  // Upload sponsor logo (admin or sponsor owner)
  app.post('/api/sponsors/upload-logo', unifiedAuth, uploadSponsorLogo.single('logo'), async (req: any, res) => {
    try {
      if (!req.file) {
        return res.status(400).json({ message: "No file uploaded" });
      }

      const logoUrl = `/assets/sponsor_logos/${req.file.filename}`;
      res.json({ logoUrl });
    } catch (error) {
